...
- [Hystrix 模型基础](#hystrix-模型基础)
- [Hystrix 工作原理](#hystrix-工作原理)
- [断路器工作原理](#断路器工作原理)
- [资源隔离技术](#资源隔离技术)
- [Spring Cloud 下 Hystrix 使用要注意的问题](#spring-cloud-下-hystrix-使用要注意的问题)
- [Hystrix 官方已死，还有什么替代方案吗？](#hystrix-官方已死还有什么替代方案吗)
- [参考资料](#参考资料)
...
在使用 Hystrix 的过程中，会对**依赖服务**的调用请求封装成**命令对象**，Hystrix 对 **命令对象**抽象了两个抽象类：`HystrixCommand` 和`HystrixObservableCommand` 。打
...
> Key 值的配置问题
> 默认配置：上述 Key 值之前要加上`hystrix.command.default.`前缀拼接
> 实例配置：上述 Key 值之前要加上`hystrix.command..` 前缀拼接
